...
IDM-style native Windows download manager. The default backend is the C
...
.\build-llvm.ps1
build-llvm\IDM-C.exe
```

This creates `build-llvm\idm.exe` and `build-llvm\IDM-C.exe` with
`clang-cl.exe`. The GUI launches `idm.exe` by default. To explicitly use the
Rust worker, place `idm-rust.exe` beside the GUI and set `$env:IDM_BACKEND =
'rust'`.
...
```powershell
build-llvm\idm.exe https://example.com/file.iso
build-llvm\idm.exe https://example.com/file.iso D:\Downloads\file.iso --connections 8
build-llvm\idm.exe --help
```
...
```powershell
.\build-llvm.ps1
iscc packaging\windows\idm-c.iss
```
...
git remote add origin https://github.com/YOUR-USER/idm-c.git
git branch -M main
